...
void ECAN_Init(void)
{
   ...
   // Enter CAN module into config mode
    ...
    PIE5bits.RXB0IE = 1;     //使能接收缓冲器0的接收中断
    IPR5bits.RXB0IP = 1;     // 接收缓冲器0的接收中断设为高优先级   
    PIE5bits.ERRIE = 1; //CAN总线错误中断
    IPR5bits.ERRIP = 1; //CAN总线错误中断设为高优先级
#endif
...
void CAN_TX_Test(void)
{
	...
	g_tCanTxMsg.Data[0] = SysRunData.PedalSta[PEDAL_LEFT];
	g_tCanTxMsg.Data[1] = SysRunData.PedalSta[PEDAL_RIGHT];	
...
/*
*********************************************************************************************************
*	函 数 名: CAN_TX_TestInfo
*	功能说明: CAN发送测试信息
*	形    参: 测试信息ID 0，左侧踏板伸出正常
*                       1，左侧踏板伸出异常
*                       2，左侧踏板缩回正常
*                       3，左侧踏板缩回异常
* 
...
void CAN_TX_TestInfo(TEST_INFO_ID_E TestInfoID)
{
	...
	g_tCanTxMsg.DLC = 0X03;				
	if(TestInfoID == INFO_ID_LEFT_PEDAL_OUT_NORMAL)
    {
        g_tCanTxMsg.Data[0] = 0x01;
        g_tCanTxMsg.Data[1] = 0x01;
        g_tCanTxMsg.Data[2] = 0x00;
    }
    else if(TestInfoID == INFO_ID_LEFT_PEDAL_OUT_ERROR)
    {
        g_tCanTxMsg.Data[0] = 0x01;
        g_tCanTxMsg.Data[1] = 0x01;
        g_tCanTxMsg.Data[2] = 0x01;		
    }
    else if(TestInfoID == INFO_ID_LEFT_PEDAL_BACK_NORMAL)
    {
        g_tCanTxMsg.Data[0] = 0x01;
        g_tCanTxMsg.Data[1] = 0x02;
        g_tCanTxMsg.Data[2] = 0x00;	
    }
    else if(TestInfoID == INFO_ID_LEFT_PEDAL_BACK_ERROR)
    {
        g_tCanTxMsg.Data[0] = 0x01;
        g_tCanTxMsg.Data[1] = 0x02;
        g_tCanTxMsg.Data[2] = 0x01;	
    }
    else if(TestInfoID == INFO_ID_RIGHT_PEDAL_OUT_NORMAL)
    {
        g_tCanTxMsg.Data[0] = 0x02;
        g_tCanTxMsg.Data[1] = 0x01;
        g_tCanTxMsg.Data[2] = 0x00;
    }
    else if(TestInfoID == INFO_ID_RIGHT_PEDAL_OUT_ERROR)
    {
        g_tCanTxMsg.Data[0] = 0x02;
        g_tCanTxMsg.Data[1] = 0x01;
        g_tCanTxMsg.Data[2] = 0x01;		
    }
    else if(TestInfoID == INFO_ID_RIGHT_PEDAL_BACK_NORMAL)
    {
        g_tCanTxMsg.Data[0] = 0x02;
        g_tCanTxMsg.Data[1] = 0x02;
        g_tCanTxMsg.Data[2] = 0x00;	
    }
    else if(TestInfoID == INFO_ID_RIGHT_PEDAL_BACK_ERROR)
    {
        g_tCanTxMsg.Data[0] = 0x02;
        g_tCanTxMsg.Data[1] = 0x02;
        g_tCanTxMsg.Data[2] = 0x01;	
...
{
    uint8_t LastKnowNPORTB;
    
    if(INTCONbits.RBIF == 1)//RB端口电平变化中断
    {
        LastKnowNPORTB = PORTB;//读取端口以匹配一下锁存器
        NOP();
        INTCONbits.RBIF = 0;
    }
