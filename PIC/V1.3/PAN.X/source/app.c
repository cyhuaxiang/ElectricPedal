...
*                                改宏定义即可。
*       V1.2    2017-09-27   cy  解决快速开关门踏板不能停止的问题。解决踏板已经伸
*                                出执行开门动作或踏板已经缩回执行关门动作导致误判
*                                断为堵转情况的问题。
*       V1.3    2017-09-27   cy  加入遥控功能，按解锁键踏板全部伸出，15s没有开门动作踏板自动缩回。按上锁键踏板全部缩回。
...
static void RemoteCtrl_Process(void);
...
{
    bsp_DetectKey();
    if(SysRunData.RemoteCtrlSta == UNLOCK)
    {
        if(++SysRunData.UnlockWaitCnt > UNLOCK_NO_OPERATION_TIME)
        {
            SysRunData.UnlockWaitCnt = 0;
            if((SysRunData.PedalSta[PEDAL_LEFT] == PEDAL_NORMAL_WAIT_BACK) && (SysRunData.PedalSta[PEDAL_RIGHT] == PEDAL_NORMAL_WAIT_BACK))
            {
                LeftDoorClose();
                RightDoorClose();
            }
            else if((SysRunData.PedalSta[PEDAL_LEFT] == PEDAL_UP_LIMIT) && (SysRunData.PedalSta[PEDAL_RIGHT] == PEDAL_NORMAL_WAIT_BACK))
            {
                RightDoorClose();
            }
            else if((SysRunData.PedalSta[PEDAL_LEFT] == PEDAL_NORMAL_WAIT_BACK) && (SysRunData.PedalSta[PEDAL_RIGHT] == PEDAL_UP_LIMIT))
            {
                LeftDoorClose();
            }
        } 
    ...
    if(LeftDoorIsOpen)//左边有门打开
    {
        switch(SysRunData.PedalSta[PEDAL_LEFT])
        {
            case PEDAL_DOWN_LIMIT:
                if(SysRunData.PedalRunCnt[PEDAL_LEFT] > PEDAL_FAST_RESPONSE_TIME) 
                {
                    ...
                    if(SysRunData.PedalRunCnt[PEDAL_LEFT] > CURRENT_SAMPLE_DELAY_TIME)
                    {
                        if(SysRunData.ADC_Buf[ADC_CHS_CURRENT_L] > PEDAL_STALL_CURRENT_VAL)//电机产生大电流
                        {
                            if(SysRunData.PedalRunCnt[PEDAL_LEFT] > PEDAL_RUN_NORMAL_TIME) //踏板动作时间大于设定值 
                            {
                                ...
                                CAN_TX_TestInfo(INFO_ID_LEFT_PEDAL_OUT_NORMAL);
                            }
                            else  //遇到障碍物
                            {
                                if(SysRunData.PedalRunCnt[PEDAL_LEFT] < PEDAL_RUN_ERROR_TIME) //踏板异常堵转时间
                                {
                                    Pedal_Stop(PEDAL_LEFT);//左踏板动作到位
                                    SysRunData.PedalRunCnt[PEDAL_LEFT] = 0;
                                    SysRunData.PedalSta[PEDAL_LEFT] = PEDAL_NORMAL_WAIT_BACK;
...
//                    StallCurrent = GetADC(ADC_CHS_CURRENT_L);
                    if(SysRunData.PedalRunCnt[PEDAL_LEFT] > CURRENT_SAMPLE_DELAY_TIME)
                    {
                        if(SysRunData.ADC_Buf[ADC_CHS_CURRENT_L] > PEDAL_STALL_CURRENT_VAL)//电机产生大电流
                        {
                            if(SysRunData.PedalRunCnt[PEDAL_LEFT] > PEDAL_RUN_NORMAL_TIME) //踏板动作时间大于设定值 
                            {
                                ...
                                SysRunData.PedalWaitSleepCnt = 0;
                                SysRunData.PedalSta[PEDAL_LEFT] = PEDAL_UP_LIMIT;
                                CAN_TX_TestInfo(INFO_ID_LEFT_PEDAL_BACK_NORMAL);
                            }
                            else//遇到障碍物
                            {
                                if(SysRunData.PedalRunCnt[PEDAL_LEFT] < PEDAL_RUN_ERROR_TIME) //踏板异常堵转时间
    ...
    if(RightDoorIsOpen)//右边有门打开
    {
        ...
        {
            case PEDAL_DOWN_LIMIT:
                if(SysRunData.PedalRunCnt[PEDAL_RIGHT] > PEDAL_FAST_RESPONSE_TIME) 
                {
                    if(SysRunData.ADC_Buf[ADC_CHS_CURRENT_R] > PEDAL_STALL_CURRENT_VAL)//电机产生大电流
                    {
                        Pedal_Stop(PEDAL_RIGHT);//右踏板动作到位
...
//                    StallCurrent = GetADC(ADC_CHS_CURRENT_R);
                    if(SysRunData.PedalRunCnt[PEDAL_RIGHT] > CURRENT_SAMPLE_DELAY_TIME)
                    {
                        if(SysRunData.ADC_Buf[ADC_CHS_CURRENT_R] > PEDAL_STALL_CURRENT_VAL)//电机产生大电流
                        {
                            if(SysRunData.PedalRunCnt[PEDAL_RIGHT] > PEDAL_RUN_NORMAL_TIME) //踏板动作时间大于设定值 
                            {
                                ...
                                CAN_TX_TestInfo(INFO_ID_RIGHT_PEDAL_OUT_NORMAL);
                            }
                            else  //遇到障碍物
                            {
                                if(SysRunData.PedalRunCnt[PEDAL_RIGHT] < PEDAL_RUN_ERROR_TIME) //踏板异常堵转时间
                                {
                                    Pedal_Stop(PEDAL_RIGHT);//右踏板动作到位
                                    SysRunData.PedalRunCnt[PEDAL_RIGHT] = 0;
                                    SysRunData.PedalSta[PEDAL_RIGHT] = PEDAL_NORMAL_WAIT_BACK;
            ...
            case PEDAL_ERROR_BACK_TIMEOUT:   
                ...
                {
                    SysRunData.PedalSta[PEDAL_RIGHT] = PEDAL_ERROR_OUT_TIMEOUT;    
                    Pedal_Stop(PEDAL_RIGHT);//右踏板遇到障碍物停止
                    SysRunData.PedalRunCnt[PEDAL_RIGHT] = 0;
                }        
                break; 
            default:
...
//                    StallCurrent = GetADC(ADC_CHS_CURRENT_R);
                    if(SysRunData.PedalRunCnt[PEDAL_RIGHT] > CURRENT_SAMPLE_DELAY_TIME)
                    {
                        if(SysRunData.ADC_Buf[ADC_CHS_CURRENT_R] > PEDAL_STALL_CURRENT_VAL)//电机产生大电流
                        {
                            if(SysRunData.PedalRunCnt[PEDAL_RIGHT] > PEDAL_RUN_NORMAL_TIME) //踏板动作时间大于设定值 
                            {
                                ...
                                SysRunData.PedalWaitSleepCnt = 0;
                                SysRunData.PedalSta[PEDAL_RIGHT] = PEDAL_UP_LIMIT;
                                CAN_TX_TestInfo(INFO_ID_RIGHT_PEDAL_BACK_NORMAL);
                            }
                            else//遇到障碍物
                            {
                                if(SysRunData.PedalRunCnt[PEDAL_RIGHT] < PEDAL_RUN_ERROR_TIME) //踏板异常堵转时间
    ...
    if((SysRunData.PedalWaitBackCnt[PEDAL_LEFT] == 0) && (SysRunData.PedalWaitBackCnt[PEDAL_RIGHT] == 0) &&
            (SysRunData.PedalRunCnt[PEDAL_LEFT] == 0) && (SysRunData.UnlockWaitCnt == 0))
    {
