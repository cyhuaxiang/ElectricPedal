        ...
        switch(SysRunData.PedalSta[PEDAL_LEFT])
        {
            case PEDAL_UP_LIMIT:
                if(SysRunData.PedalRunCnt[PEDAL_LEFT] < PEDAL_RUN_TIMEOUT_TIME)//动作未超时
                {
                    ...
                    PedalCurrentAcqDelay(PEDAL_LEFT);//踏板电流采集延时
        ...
        switch(SysRunData.PedalSta[PEDAL_RIGHT])
        {
            case PEDAL_UP_LIMIT:
                if(SysRunData.PedalRunCnt[PEDAL_RIGHT] < PEDAL_RUN_TIMEOUT_TIME)//动作未超时
                {
                    ...
                    SysRunData.PedalWaitSleepCnt = 0;
                    PedalCurrentAcqDelay(PEDAL_RIGHT);//踏板电流采集延时
...
/*
*********************************************************************************************************
*	函 数 名: PedalCurrentAcqDelay
*	功能说明: 踏板电流采集延时
*	形    参: dir:0,左踏板 1，右踏板 sta:0，失能 1，使能
*	返 回 值: 无
*********************************************************************************************************
*/
static void PedalCurrentAcq(uint8_t dir, uint8_t sta)
{
    if(dir == PEDAL_LEFT)
    {
        if(sta == ENABLE)
        {
            SysRunData.CurrentAcqSta |= (1<<0);//允许左边电流进行采样
        }
        else
        {
            SysRunData.CurrentAcqSta &= ~(1<<0);//允许左边电流进行采样
    ...
    else if(dir == PEDAL_RIGHT)
    {
        if(sta == ENABLE)
        {
            SysRunData.CurrentAcqSta |= (1<<1);//允许左边电流进行采样
        }
        else
        {
            SysRunData.CurrentAcqSta &= ~(1<<1);//允许左边电流进行采样
...
static void PedalCurrentAcqDelay(uint8_t dir)
{
    if(dir == PEDAL_LEFT)
    {
        if(++SysRunData.CurrentDelayAcqCnt[PEDAL_LEFT] >= CURRENT_SAMPLE_DELAY_TIME)//电流采集延时500ms采集
        {
            SysRunData.CurrentDelayAcqCnt[PEDAL_LEFT] = 0;
            PedalCurrentAcq(PEDAL_LEFT,ENABLE);//允许左边电流进行采样
        }
    }
    else if(dir == PEDAL_RIGHT)
    {
        if(++SysRunData.CurrentDelayAcqCnt[PEDAL_RIGHT] >= CURRENT_SAMPLE_DELAY_TIME)//电流采集延时500ms采集
        {
            SysRunData.CurrentDelayAcqCnt[PEDAL_RIGHT] = 0;
            PedalCurrentAcq(PEDAL_RIGHT,ENABLE);//允许左边电流进行采样
        }
    }
}
/*
